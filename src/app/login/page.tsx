...
export default function LoginPage() {
  ...
  return (
    <BasicLayout>
      <main className="container-pk grid min-h-[75vh] items-center py-16">
        <section className="mx-auto w-full max-w-md border border-[var(--border)] bg-white p-8">
          <p className="pk-kicker">Welcome back</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-tight">Sign in to {BRAND.name}</h1>
          <p className="mt-3 text-sm text-[var(--muted)]">Manage contributions, campaigns, and credits in one place.</p>
          ...
          <Button variant="secondary" className="w-full" onClick={() => void google()}>Continue with Google</Button>
          <p className="mt-6 text-sm">
            New to {BRAND.name}?{" "}
            <Link href="/register" className="font-semibold text-[var(--pk-blue)]">Create an account</Link>
          </p>
        </section>
      </main>
    </BasicLayout>
